...
int main(int argc, char **argv)
{
	...
	*/
	...
	// First and Second thread render the left and right sides
	std::thread t1([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = 0; y < windowHeight * 0.5; y++)
		{
			for (int x = 0; x < windowWidth * 0.5; x++)
			{
				#ifdef TIMING_PER_PIXEL
				...
				if (x % 2 == 0 && y % 2 == 0) {
					output = g_scene.trace(float(x - windowWidth / 2) / (windowWidth / 2), -float(y - windowHeight / 2) / (windowHeight / 2)*((float(windowHeight) / windowWidth)));
				}
				...
				*image.data(x, y, 0, 2) = output.b * 255;
				...
				#ifdef TIMING_PER_PIXEL
					long long pixelTime = timer.getTicks() - startPixelTime;
					timingData[x + y*windowWidth] = pixelTime;
					if (pixelTime < minPixelTiming)
						minPixelTiming = pixelTime;
					if (pixelTime > maxPixelTiming)
						maxPixelTiming = pixelTime;
				#endif
			}
	...
	std::thread t2([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = 0; y < windowHeight * 0.5; y++)
		{
			for (int x = windowWidth * 0.75; x < windowWidth * 1.0; x++)
			{
#ifdef TIMING_PER_PIXEL
				long long startPixelTime = timer.getTicks();
#endif
	...
	// Third and Fourth thread render the Middle
	std::thread t3([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = 0; y < windowHeight * 0.75; y++)
		{
			for (int x = windowWidth * 0.25; x < windowWidth * 0.5; x++)
			{
#ifdef TIMING_PER_PIXEL
				long long startPixelTime = timer.getTicks();
#endif
	...
	std::thread t4([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = 0; y < windowHeight * 0.75; y++)
		{
			for (int x = windowWidth * 0.5; x < windowWidth * 0.75; x++)
			{
#ifdef TIMING_PER_PIXEL
				long long startPixelTime = timer.getTicks();
#endif
	...
	// Fifth to Eighth thread render the Floor
	std::thread t5([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = windowHeight * 0.5; y < windowHeight * 1.0; y++)
		{
			for (int x = 0; x < windowWidth * 0.25; x++)
			{
#ifdef TIMING_PER_PIXEL
				long long startPixelTime = timer.getTicks();
#endif
	...
	std::thread t6([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = windowHeight * 0.75; y < windowHeight * 1.0; y++)
		{
			for (int x = windowWidth * 0.25; x < windowWidth * 0.5; x++)
			{
#ifdef TIMING_PER_PIXEL
	...
	std::thread t7([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = windowHeight * 0.75; y < windowHeight * 1.0; y++)
		{
			for (int x = windowWidth * 0.5; x < windowWidth * 0.75; x++)
	...
	std::thread t8([&main_disp, &image, &timer, &startTime]() -> void {
		// Primary loop through all screen pixels.
		for (int y = windowHeight * 0.5; y < windowHeight * 1.0; y++)
		{
			for (int x = windowWidth * 0.75; x < windowWidth * 1.0; x++)
	...
	for (int y = 0; y < windowHeight - 1; y++)
	{
		for (int x = 0; x < windowWidth - 1; x++)
		{
			if (x % 2 != 0) {
				...
				*image.data(x, y, 0, 0) = (*image.data(x - 1, y, 0, 0) + *image.data(x + 1, y, 0, 0)) / 2;
				*image.data(x, y, 0, 1) = (*image.data(x - 1, y, 0, 1) + *image.data(x + 1, y, 0, 1)) / 2;
				*image.data(x, y, 0, 2) = (*image.data(x - 1, y, 0, 2) + *image.data(x + 1, y, 0, 2)) / 2;
			}
	...
	// Interpolate Remaining Pixels (Y)
	for (int y = 0; y < windowHeight - 1; y++)
	{
		for (int x = 0; x < windowWidth - 1; x++)
		{
			if (y % 2 != 0) {
				...
				*image.data(x, y, 0, 0) = (*image.data(x, y - 1, 0, 0) + *image.data(x, y + 1, 0, 0)) / 2;
				*image.data(x, y, 0, 1) = (*image.data(x, y - 1, 0, 1) + *image.data(x, y + 1, 0, 1)) / 2;
				*image.data(x, y, 0, 2) = (*image.data(x, y - 1, 0, 2) + *image.data(x, y + 1, 0, 2)) / 2;
			}
